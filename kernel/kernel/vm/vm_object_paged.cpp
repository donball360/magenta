...
status_t VmObjectPaged::GetPageLocked(uint64_t offset, uint pf_flags, vm_page_t** const page_out, paddr_t* const pa_out) TA_REQ(lock_) {
    ...
    if (parent_) {
        ...
        // make sure we dont cause the parent to fault in new pages, just ask for any that already exist
        ...
        if (status == NO_ERROR) {
            ...
            __UNUSED auto status = page_list_.AddPage(p_clone, offset);
    ...
    // if we're read faulting, we don't already have a page, and the parent doesnt have it,
